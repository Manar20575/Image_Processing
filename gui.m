...
% Last Modified by GUIDE v2.5 17-Dec-2023 19:27:44
...
if handles.s_p.Value
    modified_img = saltAndPepper(modified_img,ps_val,pp_val);
    axes(handles.modified);
    imshow(modified_img);
elseif handles.uniform.Value
    modified_img = uniform_noise(modified_img,a_val,b_val);
    axes(handles.modified);
    imshow(modified_img);
elseif handles.g.Value
    modified_img = gaussian_noise(modified_img,mean_val,std_val);
    axes(handles.modified);
    imshow(modified_img);
end
...
global n_max_val;
global n_min_val;
if handles.hist_h.Value
    modified_img = Histogram(modified_img);
    axes(handles.modified_img);
elseif handles.hist_equal.Value
...
function mean_Callback(hObject, eventdata, handles)
    global mean_val;
    mean_val = str2double(get(handles.mean,'String'));
...
function std_Callback(hObject, eventdata, handles)
    global std_val;
    std_val = str2double(get(handles.std,'String'));
...
% --- Executes on button press in ift.
function ift_Callback(hObject, eventdata, handles)
global image;
global modified_img;
modified_img = ift(image);
axes(handles.modified);
imshow(modified_img);
